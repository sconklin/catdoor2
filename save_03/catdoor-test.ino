...
int getangle(String command)
{
    ...
    switch (chan) {
        case 0:
            angle = (analogRead(SERVO_1_FEEDBACK_PIN) * 360) / 4096;
            break;
        case 1:
            angle = (analogRead(SERVO_2_FEEDBACK_PIN) * 360) / 4096;
            break;
        default:
            angle = -1;
            break;
...
void setup() {
    ...
    Particle.function("getangle", getangle);
...
// assume 1500 is middle of dead zone, and the dead zone is 60 in width
//int highclamp = 1550; //1570;
//int lowclamp = 1465; //1290;
int deadzonebottom = 1470;
int deadzonetop = 1530;

int highclamp = 1540; //1570;
int lowclamp = 1450; //1290;


// PID coefficients
double kp = 0.8; // 0.5
double ki = 0.0;//-0.00007;//.0008; // sign is correct
double kd = 0.05; //0.05;

#define MAX_CONTROL_LOOP_COUNT 20000
...
int movemotors(int position[]) {
    ...
    Serial.printlnf("currentpos %d, %d", current_position[0], current_position[1]);
    ...
    while (true) {
        ...
        for (i=0; i<NUMSERVOS; i++) {
            ...
            rawerror[i] = current_position[i] - position[i];
            ...
            // Now eliminate the deadzone
            if ((pwm[i] > SRV_STOP_US) && (pwm[i] < deadzonetop)) {
                pwm[i] += 20;
            }
            if ((pwm[i] < SRV_STOP_US) && (pwm[i] > deadzonebottom)) {
                pwm[i] -= 20;
            }
